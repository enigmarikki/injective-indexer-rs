...
pub use crate::proto::injective::stream::v1beta1::{
    StreamRequest, StreamResponse, TradesFilter, PositionsFilter, OrdersFilter,
    OrderbookFilter, BankBalancesFilter, SubaccountDepositsFilter, OraclePriceFilter,
};
...
pub enum KafkaPayload {
    ...
    SpotOrderbooks(Vec<OrderbookPayload>),
    DerivativeOrderbooks(Vec<OrderbookPayload>),
...
impl From<crate::proto::injective::stream::v1beta1::StreamResponse> for Vec<KafkaMessage> {
    fn from(response: crate::proto::injective::stream::v1beta1::StreamResponse) -> Self {
        ...
        if !response.bank_balances.is_empty() {
            let bank_balances = response.bank_balances.into_iter()
                .map(|b| BankBalancePayload {
                    account: b.account,
                    balances: b.balances.into_iter()
                        .map(|c| CoinPayload {
                            denom: c.denom,
                            amount: c.amount,
                        })
            ...
            messages.push(KafkaMessage {
                message_type: MessageType::BankBalance,
                block_height,
                block_time,
                payload: KafkaPayload::BankBalances(bank_balances),
            });
        }

        // Process subaccount deposits
        if !response.subaccount_deposits.is_empty() {
            let subaccount_deposits = response.subaccount_deposits.into_iter()
                .flat_map(|sd| {
                    let subaccount_id = sd.subaccount_id.clone();
                    sd.deposits.into_iter().map(move |d| {
                        SubaccountDepositPayload {
                            subaccount_id: subaccount_id.clone(),
                            denom: d.denom,
                            available_balance: d.deposit.as_ref().map_or_else(|| "0".to_string(), |dep| dep.available_balance.clone()),
                            total_balance: d.deposit.as_ref().map_or_else(|| "0".to_string(), |dep| dep.total_balance.clone()),
                        }
                    })
                })
                .collect();
            ...
            messages.push(KafkaMessage {
                message_type: MessageType::SubaccountDeposit,
                block_height,
                block_time,
                payload: KafkaPayload::SubaccountDeposits(subaccount_deposits),
            });
        }

        // Process spot trades
        if !response.spot_trades.is_empty() {
            let spot_trades = response.spot_trades.into_iter()
        ...
        if !response.derivative_trades.is_empty() {
            let derivative_trades = response.derivative_trades.into_iter()
                .map(|t| DerivativeTradePayload {
                    ...
                    position_delta: PositionDeltaPayload {
                        ...
                        execution_quantity: t.position_delta.as_ref().map_or_else(|| "0".to_string(), |pd| pd.execution_quantity.clone()),
                        execution_margin: t.position_delta.as_ref().map_or_else(|| "0".to_string(), |pd| pd.execution_margin.clone()),
                        execution_price: t.position_delta.as_ref().map_or_else(|| "0".to_string(), |pd| pd.execution_price.clone()),
                    },
                    payout: t.payout,
                    fee: t.fee,
                    order_hash: t.order_hash,
        ...
        if !response.spot_orders.is_empty() {
            let spot_orders = response.spot_orders.into_iter()
                .map(|o| {
                    ...
                    let market_id = o.order.as_ref().map_or_else(|| "".to_string(), |order| order.market_id.clone());
                    ...
                    SpotOrderPayload {
                        ...
                        cid: o.cid,
                        market_id,
                        subaccount_id: o.order.as_ref().and_then(|order| order.order.as_ref()).and_then(|order| order.order_info.as_ref()).map_or_else(|| "".to_string(), |info| info.subaccount_id.clone()),
                        price: order_info.and_then(|order| order.order_info.as_ref()).map_or_else(|| "0".to_string(), |info| info.price.clone()),
                        quantity: order_info.and_then(|order| order.order_info.as_ref()).map_or_else(|| "0".to_string(), |info| info.quantity.clone()),
                        fillable: o.order.as_ref().and_then(|order| order.order.as_ref()).map_or_else(|| "0".to_string(), |o| o.fillable.clone()),
                        is_buy: order_info.and_then(|order| order.order_info.as_ref()).map_or(false, |_| {
                            match o.order.as_ref().and_then(|o| o.order.as_ref()).map(|o| o.order_type) {
                                Some(1) | Some(3) | Some(5) | Some(7) | Some(9) => true, // BUY, STOP_BUY, TAKE_BUY, BUY_PO, BUY_ATOMIC
                                _ => false,
                            }
                        }),
                        order_type: o.order.as_ref().and_then(|o| o.order.as_ref()).map_or("Unknown".to_string(), |o| {
                            match o.order_type {
        ...
        if !response.derivative_orders.is_empty() {
            let derivative_orders = response.derivative_orders.into_iter()
                .map(|o| {
                    let status_str = match o.status {
                        0 => "Unspecified",
                        1 => "Booked",
                    ...
                    DerivativeOrderPayload {
                        ...
                        cid: o.cid,
                        market_id,
                        subaccount_id: order_info_inner.map_or_else(|| "".to_string(), |info| info.subaccount_id.clone()),
                        price: order_info_inner.map_or_else(|| "0".to_string(), |info| info.price.clone()),
                        quantity: order_info_inner.map_or_else(|| "0".to_string(), |info| info.quantity.clone()),
                        margin: order_info.map_or_else(|| "0".to_string(), |info| info.margin.clone()),
                        fillable: order_info.map_or_else(|| "0".to_string(), |info| info.fillable.clone()),
        ...
        if !response.spot_orderbook_updates.is_empty() {
            let spot_orderbooks = response.spot_orderbook_updates.into_iter()
                .map(|o| OrderbookPayload {
                    market_id: o.orderbook.as_ref().map_or_else(|| "".to_string(), |ob| ob.market_id.clone()),
                    buy_levels: o.orderbook.as_ref().map_or_else(Vec::new, |ob| 
                        ob.buy_levels.iter().map(|l| PriceLevelPayload {
                            price: l.p.clone(),
                            quantity: l.q.clone(),
                        }).collect()
                    ),
                    sell_levels: o.orderbook.as_ref().map_or_else(Vec::new, |ob| 
                        ob.sell_levels.iter().map(|l| PriceLevelPayload {
                            price: l.p.clone(),
                            quantity: l.q.clone(),
                        }).collect()
                    ),
            ...
            messages.push(KafkaMessage {
                message_type: MessageType::SpotOrderbook,
                block_height,
                block_time,
                payload: KafkaPayload::SpotOrderbooks(spot_orderbooks),
            });
        }

        // Process derivative orderbook updates
        if !response.derivative_orderbook_updates.is_empty() {
            let derivative_orderbooks = response.derivative_orderbook_updates.into_iter()
                .map(|o| OrderbookPayload {
                    market_id: o.orderbook.as_ref().map_or_else(|| "".to_string(), |ob| ob.market_id.clone()),
                    buy_levels: o.orderbook.as_ref().map_or_else(Vec::new, |ob| 
                        ob.buy_levels.iter().map(|l| PriceLevelPayload {
            ...
            messages.push(KafkaMessage {
                message_type: MessageType::DerivativeOrderbook,
                block_height,
                block_time,
                payload: KafkaPayload::DerivativeOrderbooks(derivative_orderbooks),
            });
        }

        // Process positions
        if !response.positions.is_empty() {
            let positions = response.positions.into_iter()
            ...
            messages.push(KafkaMessage {
                ...
                block_height,
                block_time,
                payload: KafkaPayload::Positions(positions),
            });
        }

        // Process oracle prices
        if !response.oracle_prices.is_empty() {
            let oracle_prices = response.oracle_prices.into_iter()
            ...
            messages.push(KafkaMessage {
                message_type: MessageType::OraclePrice,
                block_height,
                block_time,
                payload: KafkaPayload::OraclePrices(oracle_prices),
            });
        }
